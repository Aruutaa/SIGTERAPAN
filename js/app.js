...
const locations = [
    ...
    { name: "Taman Payung Maskan UGM", coord: [-7.774717, 110.380989], type: "leisure", rating: "4.5 ⭐", jam: "Buka 24 Jam", alamat: "Bulaksumur", image: "data/images/Taman Payung Seberang Maskan UGM.jpg" },
...
window.toggleAnalyticsLayers = function() {
    ...
    // A. Heatmap Stres & Kebisingan (Simulasi Prediksi IoT)
    if(document.getElementById('layer-heatmap').checked) {
        if(typeof L.heatLayer !== 'undefined') {
            let heatPoints = [];
            // Generate mock data: area padat sekitar jalan raya Seturan & Babarsari
            for(let i=0; i<150; i++) {
                heatPoints.push([
                    -7.76 + (Math.random()*0.025), 
                    110.40 + (Math.random()*0.025), 
                    Math.random() // intensitas stres
                ]);
            }
            ...
            document.getElementById('system-status').textContent = "🔥 Peta Heatmap Prediksi Stres/Bising Kota Ditampilkan.";
        } else {
            console.warn("Library Leaflet.heat belum dimuat!");
        }
    }

    // B. Analisis Akses Ruang Hijau (Radius 300m, 500m, 1000m)
    if(document.getElementById('layer-buffer').checked) {
        locations.forEach(loc => {
            if(loc.type === 'leisure') {
                // Buffer 1 KM (Jangkauan Kendaraan Ringan)
                L.circle(loc.coord, {radius: 1000, color: '#0ea5e9', weight: 1, fillOpacity: 0.05, dashArray: '4,4'}).addTo(analyticsLayersGroup);
                // Buffer 500m (Jangkauan Jalan Santai)
                L.circle(loc.coord, {radius: 500, color: '#10b981', weight: 2, fillOpacity: 0.1}).addTo(analyticsLayersGroup);
                // Buffer 300m (Jangkauan Ideal Pejalan Kaki/Smart Walking)
                L.circle(loc.coord, {radius: 300, color: '#059669', weight: 3, fillOpacity: 0.2}).addTo(analyticsLayersGroup);
            }
        });
        document.getElementById('system-status').textContent = "🎯 Analisis Radius Jangkauan RTH Kota (300m, 500m, 1km) Aktif.";
    }
}
...
function renderLeafletFeatures() {
    ...
    locations.forEach(loc => {
        ...
        if (layerVisible) {
            ...
            // Klik memicu Papan Informasi Sidebar
            marker.on('click', () => window.updateSidebarInfo(loc)); 
